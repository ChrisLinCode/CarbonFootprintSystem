...
public class Train_model_CKIP_fasttext {

    private static JFastText fastText;


    // ============ 共用工具方法 ============
    // 單筆分詞（用於預測時）：改為呼叫 CKIPClientAPI.segment()
    private static String callCkipSegmentation(String text) throws Exception {
        return CKIPClientAPI.segment(text);
    }

// 批量分詞：改為呼叫 CKIPClientAPI.segmentBatch()
    private static List<String> callCkipSegmentationBatch(List<String> texts) throws Exception {
        return CKIPClientAPI.segmentBatch(texts);
    }
    ...
    private static Instances preprocessDataBatch(Instances data) throws Exception {
        ...
        // 使用批量分詞
        List<String> processedTexts = callCkipSegmentationBatch(originalTexts);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 交叉驗證 (確保折數不大於資料數量)
            int numFolds = Math.min(10, vectorizedData.numInstances());
            if (numFolds < 2) {
                throw new IllegalArgumentException("Not enough instances for cross validation!");
            }
    ...
    public static class UnifiedModel implements Serializable {
        ...
        private Instances preprocessData(String data) throws Exception {
            ...
            String processedText = callCkipSegmentation(data);
            if (processedText.isEmpty()) {
                processedText = "empty";
            }
            double[] values = new double[2];
